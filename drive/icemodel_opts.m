...
opts.pathoutput = fullfile(getenv('ICEMODELOUTPUTPATH'), sitename, simmodel, userdata);
...
% build the output folders if they don't already exist. This creates one folder
% for each simulation year in pathoutput/
if opts.savedata
   if ~isfolder(opts.pathoutput)
      warning('output folders do not exist, creating them in %s', opts.pathoutput)
...
% to reconcile sector and site-scale runs, could try to put the common logic for
% met file and output file names in the first part of the file name and use
% empty chars for the non-common or an if-else. For now, set the metfname and
% outfname in the run script for sector scale runs otherwise 
% opts.metfname = fullfile(opts.pathinput, 'met', ['met_' metname '_<other parts>.mat']);
% opts.outfname = fullfile(opts.pathoutput, [simmodel '_' sitename '_<other parts>']);
...
if strcmp(sitename, 'sector')
   ...
   opts.vars1 = {'Tsfc', 'Qm', 'Qf', 'Qe', 'Qh', 'Qc', 'chi', 'balance', 'dt_sum'};
   opts.vars2 = {'Tice', 'f_ice', 'f_liq', 'df_liq', 'df_drn', 'df_evp', 'Sc', 'errH', 'errT'};
   ...
   if strcmpi(forcings, 'kanl') && any(ismember(sitename, {'ak4','upperbasin'}))
      metname = 'kanl';
   elseif strcmpi(forcings, 'kanm') && any(ismember(sitename, {'slv1','slv2','behar'}))
      metname = 'kanm';
   else
      metname = sitename;
   end
