...
% cvconvert - Convert constituent volume properties among several dimensions
% including mass, volume fraction, bulk density, total density, and volume in
% each CV.
%
% Syntax: [out1, ..., outN] = cvconvert(FROM, TO, CV, CONSTS, in1, ..., inN)
% Converts numeric values in1, ..., inN from their value in FROM to their value
% in TO using the control volumes in CV and the rquired physical constants in
% CONSTS.
%
% Inputs: convertFrom - String indicating the dimension of the input quantities
...
% constants (i.e., densities, heat capacities) needed for conversions. varargin
% - One or more vectors or matrices, each corresponding to a constituent, with
% elements representing the constituent's quantity in the dimension specified by
% convertFrom.
%
% Outputs: varargout - One or more vectors or matrices, each corresponding to a
...
%
% Example: [m_liq, m_ice] = icemodel.cvconvert('volumefraction', 'mass', dz,
% [ro_liq, ro_ice], f_liq, f_ice); This converts the volume fractions of liquid
...
if ~isscalar(CV)
   assert(all(size(CV) == size(values{1})), 'Size of CV must match size of input fractions.');
end
