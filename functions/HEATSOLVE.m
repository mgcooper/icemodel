function [H,T,errH,errT,OK] = HEATSOLVE(T_old,H_old,f_wat,f_ice,f_liq, ...
      k_liq,cv_ice,cv_liq,ro_ice,ro_liq,ro_sno,cp_liq,cp_sno,dFdT,Ls,Lf, ...
      roLf,Rv,Tf,dz,delz,fn,dzdt,JJ_therm,Tsfc,Sc,fcp,TL,TH,flmin,flmax, ...
      fcpsq,ro_iwe,ro_wie)
   %HEATSOLVE Iteratively solve the nonlinear heat equation.
   ...
   while any(errT > tol) && iter < maxiter
      ...
      [aN,aP,aS,b,iM] = GECOEFS(T_old,ro_sno,cp_sno,f_liq,f_ice,cp_liq,Ls, ...
         Lf,dz,dzdt,dFdT,drovdT,TL,TH,H,H_old,Sc,k_eff,fn,delz,Tsfc,JJ_therm);

      T = TRISOLVE(-aN,aP,-aS,b);

      % back-transform the meltzone preconditioner
      [T,f_liq,OK] = MZTRANSFORM(T,T_old,f_liq,f_wat,ro_liq,Tf,TL,TH,fcp, ...
         flmin,flmax,iM);
      ...
      [f_liq,f_ice,~,T] = MELTCURVE(T,f_liq,f_ice,ro_wie,ro_iwe,Tf,fcp,fcpsq);
