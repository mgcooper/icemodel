function [f_ice, f_liq, T, TL, TH, flmin, flmax, cp_sno, k_eff, dz, fn, ...
   ...
   wcoef, scoef, ro_sno, ro_iwe, ro_wie, ice1, ice2] = ICEINIT(opts, met)
...
T_dep = Tf-T(:,1);                        % [K]
f_liq = 1./(1+(fcp.*T_dep).^2);           % [-]
g_ice = ro_i;                             % at t1, bd_ice=ro_sno
g_liq = g_ice.*(f_liq./(1-f_liq));
f_liq = g_liq./ro_liq;
f_ice = g_ice./ro_ice.*ones(JJ_therm,1);

% effective thermal cond, density, and sp. heat cap.
[k_eff, ro_sno, cp_sno] = UPDATESTATE(T, f_ice, f_liq, ro_ice, ro_liq, ...
   ro_air, cp_ice, cp_liq, k_liq, Ls, Rv, Tf);

% lower and upper melt zone temperature
TL = Tf-(2.0*Lf/(fcp*fcp*cp_ice))^(1.0/3.0);   % Eq 120
TH = Tf-cp_liq/(Lf*2.0*fcp*fcp);

% lower and upper melt zone water fractions
