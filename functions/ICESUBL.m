...
% note: if liqflag is true, f_liq must be > f_liq_min, but if liqflag is false
% and pevap > 0, it is possible that f_liq is < f_liq_min on entry to this
% function. CORRECTION - liqflag can be true and f_liq can be < f_liq_min on
% entry to this function b/c f_liq_min is based on the start of the timestep
% f_wat, and during ICEENBAL, f_liq(1) can drop below f_liq_min as long as these
% evaluate false:
...
% then we have an error, otherwise if f_ice(1)+psubl < f_min, the layers will be
% combined.
