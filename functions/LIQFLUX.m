...
% march 2023, looking at this, it should be straightforward to compute this
% after each timestep, for each layer, and whatever gets to the base, either
% release immediately, or put in a conceptual aquifer that drains slowly. but
% the main thing is that at each step, this moves water around, so f_liq
% changes, and total heat changes, and if the timestep is small enough, it could
% go at the end of each step as an update state type thing, but if that fails,
% it will need to go inside the solver
...
%             mLayerVolFracLiq,                                           ...
%             mLayerVolFracIce,                                           ...
%             retent ]            =   UPDATE_STATE(T,mLayerTheta,        ...
%                                     fcp,Tf,ro_liq,retent)
% %--------------------------------------------------------------------------
% 
