function [z, exitflag] = fsearchzero(f, z0, A, B, z, tol)
%FSEARCHZERO search for root of nonlinear function %#codegen
[a, b] = fzero_guess_to_bounds(f, z0, A, B); exitflag = -6;
if ~isnan(a)
   [z, exitflag] = fzero_brent(f, a, b, tol);
end
...
%
%
% [a, b] = fzero_guess_to_bounds(f, x) finds a < b such that f(a) and f(b) have
% different sign*, meaning a solution exists within the interval [a,b].  The
% bounds a,b are expanded outward in geometric progression from an initial guess
% for the root of f at x. If f evaluates to NaN at any point during the search,
% then a = nan and b = nan are immediately returned.  If the function is
% genuinely single-signed, or even if it is not but its values of opposite sign
% are skipped over, it is possible to enter an infinite loop.  Calling the
% function in this form is therefore not recommended unless you know the
% function will not result in such an infinite loop.
%
% [a, b] = fzero_guess_to_bounds(f, x, A, B) as above, but limits [a,b] to
% remain inside the subset [A, B].  If x is outside of [A, B], it is immediately
% moved into this range. If no sign-change is found within [A, B], then a = nan
% and b = nan are returned.  Note, as above, it is possible that a sign-change
% is skipped over as f is only evaluated at finitely many x values.
%
...
if nargin >= 4 %&& isscalar(A) && isscalar(B) % mgc removed checks
...
%% brent solver
function [b, exitflag] = fzero_brent(f, a, b, tol, varargin)
...
% mgc add exitflag that mimics fzero. since this is only called if the bound
