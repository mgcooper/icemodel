function [ice1, ice2] = icemodel(opts) %#codegen
   ...
   [f_ice, f_liq, T, TL, TH, flmin, flmax, cp_sno, k_eff, dz, fn, delz, ...
      z_therm, dz_therm, dz_spect, JJ_therm, JJ_spect, ~, Sp, scoef, ...
   ...
   [total_solar, z_spect, z_walls, spect_lower, spect_upper, solardwavl] = ...
      ...
      while iter <= maxiter
         ...
         if swd(metiter) > 0 && isicemodel
            [Sc, chi] = UPDATEEXTCOEFS(swd(metiter), albedo(metiter), ...
               z_spect, JJ_spect, dz_spect, z_therm, JJ_therm, ...
