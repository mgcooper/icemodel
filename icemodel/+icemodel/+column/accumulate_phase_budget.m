function [ledger, solid_p, liquid_p, phase_solid, phase_liquid] = ...
   ...
   %
   % Inputs
   %   ledger              - Forcing-step ledger from icemodel.column.initialize_budget_state.
   ...
   %
   % Outputs
   %   ledger              - Ledger with the phase channels accumulated.
   ...
   %   phase_liquid          close storage throughput after remeshing.
   %
   % See also: icemodel.column.initialize_budget_state, icemodel.column.integrate_column_budget,
   %  icemodel.column.accumulate_vapor_budget
   %
   %#codegen

   % Difference the checkpointed and solved states on one fixed storage basis
   % so the increment cannot absorb a change of reference density.
   [solid_0, liquid_0] = icemodel.column.integrate_column_budget(xT, xf_ice, xf_liq, dz);
   [solid_p, liquid_p] = icemodel.column.integrate_column_budget(T, f_ice, f_liq, dz);
   ...
   ledger.mass_budget_phase_solid_throughput_mwe = ...
      ledger.mass_budget_phase_solid_throughput_mwe + abs(phase_solid);
   ledger.mass_budget_phase_liquid_throughput_mwe = ...
      ledger.mass_budget_phase_liquid_throughput_mwe + abs(phase_liquid);
end
