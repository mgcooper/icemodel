function ice1 = diagnose_column_runoff(ice1, ice2, opts)
   ...
   % and runoff. Condensation above the top cell's water capacity never
   % enters the column, it runs off here with no residence time.
   %
   ...
   % Partition melt/freeze phase change.
   ...
   % df_evp stores per-cell liquid increments from surface exchange. Positive
   % values add liquid to the reservoir. Interior transport uses df_vap_liq and
   % is not included here as potential runoff.
   vapor_liquid = liquidVaporSupply(ice2, dz, n_steps);

   % Overflow is condensation the top cell's water capacity rejected.
...
function values = liquidVaporSupply(ice2, dz, n_steps)
   %LIQUIDVAPORSUPPLY Return total liquid vapor surface-exchange per step [m].
   ...
   % Sum the per-cell liquid increments over the uniform thermal grid.
...
function values = overflowSupply(ice1, dz, n_steps)
   ...
   %
   % ice1.df_rof is a top-cell liquid fraction. Scale it by the top-cell
   % thickness to compute runoff in mwe.
