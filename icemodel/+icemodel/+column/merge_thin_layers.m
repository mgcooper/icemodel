function [T, f_ice, f_liq, Sc, Sp, d_lyr, budget] = ...
   ...
   %
   % Outputs
   %   T, f_ice, f_liq - Updated column state after remeshing.
   ...
   %                     Only these removals translate the fixed grid downward.
   %                     interior_merge_count counts every other removal.
   %                     top_export_* is the mass that a surface removal
   %                     exported. A merge keeps the mean of the pair, so
   %                     top_export_* over-counts what the removed cell held,
   %                     and it is not a surface mass flux. The quantized
   %                     top_deletion_height_m is grid geometry, not mass.
   %                     Domain exchange closes as remesh_solid =
   %                     cloned_bottom_solid - merge_export_solid.
   %
