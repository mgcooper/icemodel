function [Ts, T, f_ice, f_liq, k_eff, ok_seb, ok_ieb, ok_cpl, n_iters] = ...
      solve_skin_surface_column(xTs, xT, xf_ice, xf_liq, dz, delz, ...
      fn, dt, tair, swd, lwd, albedo, wspd, ppt, tppt, psfc, ea_atm, ...
      ro_atm, cv_atm, nu_air, H_h, H_e, hv_atm, br_coefs, liqflag, ...
      chi, tol, maxiter, alpha, cpl_maxiter, cpl_Ts_tol, cpl_seb_tol, ...
   ...
   [Ts, ok_seb] = icemodel.surface.solve_surface_energy_balance( ...
      xTs, tair, swd, lwd, albedo, wspd, ppt, tppt, psfc, ea_atm, ...
      ro_atm, cv_atm, nu_air, H_h, H_e, hv_atm, br_coefs, liqflag, ...
   ...
   for cpliter = 1:cpl_maxiter
      ...
      if ~ok_ieb
         if debug
            dumpSkinEbSolveFailure("skinsolve_failed", Ts, Ts_diag, ...
               Ts_old, T, f_ice, f_liq, k_eff, dt, cpliter, ...
               cpl_maxiter, cpl_Ts_tol, cpl_seb_tol, seb_res, ...
               n_iters, ok_seb, ok_ieb, ok_cpl);
         end
         break
      end
      ...
      % SEB residual
      seb_res = abs( ...
         icemodel.surface.surface_energy_balance_residual(Ts, ...
