function [Ts, T, f_ice, f_liq, k_eff, ok_seb, ok_ieb, ok, n_iters] = ...
   ...
   [Ts, ok_seb] = icemodel.surface.solve_surface_energy_balance(xTs, tair, ...
      ...
      br_coefs, liqflag, xT, k_eff, dz, ro_sfc, snow_depth, seb_solver, ...
      debug, opts);
   ...
   for cpliter = 1:cpl_maxiter
      ...
      Ts_old = Ts;
      [Ts, ok_seb] = icemodel.surface.solve_surface_energy_balance(Ts, tair, ...
         swd, lwd, albedo, wspd, ppt, tppt, psfc, De, ea_atm, chi, roL, ...
         br_coefs, liqflag, T, k_eff, dz, ro_sfc, snow_depth, seb_solver, ...
         debug, opts);
      Ts_diag = Ts;
      ...
      seb_res = abs(icemodel.surface.surface_energy_balance_residual(Ts, ...
         tair, swd, lwd, albedo, wspd, ppt, tppt, psfc, De, ea_atm, chi, ...
         roL, br_coefs, CONDUCT(k_eff, T, dz, Ts), liqflag, ro_sfc, ...
         snow_depth, opts));
