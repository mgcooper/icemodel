function [T_sfc, T_ice, f_ice, f_liq, k_eff, U_vap, L_vap, diag] = ...
   ...
   % linearized Robin surface state, the top-node temperature, and the
   % conductive closure consistent with each other at the end of a substep.
   % solver 2 is the single-iteration special case (cpl_maxiter = 1) solver 3
   % runs the full outer iterations. The coupler makes one attempt. On a fail,
   ...
   % true here since there's no explicit T_sfc solve to fail.
   %
   % See also: icemodel,
   ...
   for cpliter = 1:settings.cpl_maxiter
      ...
      % Inner subsurface solve with updated T_sfc, Fc, Fp and checkpoint state.
      ...
      if ~ok_ieb
         if debug
            dumpIceEbSolveFailure("iceenbal_failed", ...
               T_sfc, T_sfc_diag, T_sfc_old, T_ice, f_ice, f_liq, k_eff, ...
               Sc, dt, Fc, Fp, cpliter, settings, seb_res, ok_ieb, ok_cpl, ...
               n_iters, res_hist);
         end
         break
      end
      ...
      % Diagnose T_sfc from frozen coefficients and updated (a1, T1).
   ...
   if ~(ok_ieb && ok_cpl) && debug && ok_ieb
      dumpIceEbSolveFailure("coupler_nonconvergence", ...
         T_sfc, T_sfc_diag, T_sfc_old, T_ice, f_ice, f_liq, k_eff, Sc, dt, ...
         Fc, Fp, cpliter, settings, seb_res, ok_ieb, ok_cpl, n_iters, res_hist);
   end
...
function dumpIceEbSolveFailure(reason, T_sfc, T_sfc_diag, T_sfc_old, T_ice, ...
      f_ice, f_liq, k_eff, Sc, dt, Fc, Fp, cpliter, settings, seb_res, ...
