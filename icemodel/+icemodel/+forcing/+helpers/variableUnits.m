function units = variableUnits(names)
   ...
   %  swd_cor, swu_cor             W m-2    tilt/bias-corrected shortwave
   %  shum                         kg/kg    specific humidity
   %  rh_ice                       %        RH wrt saturation over ice/water
   %  uwind, vwind                 m s-1    wind speed x/y components
   %  tilt_x, tilt_y, rot          degrees  platform tilt and azimuth
   %  tice1..ticeN, tice10m        K        ice-temperature string + 10 m firn
   %  dtice1..dticeN               m        thermistor depths of the tice string
   %
   ...
   for k = 1:numel(names)
      ...
      elseif ~isempty(regexp(name, '^dtice\d+$', 'once'))
         units(k) = "m";   % thermistor depth string of arbitrary length
      else
...
function m = canonicalUnitMap()
   ...
   keys = [ ...
      ...
      "swd_cor", "swu_cor", "shum", "rh_ice", "uwind", "vwind", ...
      "tilt_x", "tilt_y", "rot", "tice10m"];
   vals = [ ...
      "K", "K", ...
