function bands = solarElevationBands()
   %SOLARELEVATIONBANDS Solar-elevation thresholds for the swd science path.
   %
   %  bands = icemodel.forcing.reconstruct.solarElevationBands()
   %
   % Role
   %  Dedicated function (like physicalBounds and bucketEdges) that holds
   %  every solar-elevation threshold the swd science path uses. The
   %  darkness pre-pass and the elevation-binned proxy calibration read the
   ...
   %        real diffuse twilight light, so the code does not zero those
   %        samples. They reach the normal fill tiers.
   ...
   %        morning shoulder (kanl hour 7: observed 81 vs MAR 171). The
   %        bins therefore isolate deep darkness [-90, -6), the twilight
   %        band [-6, 0), the low-sun shoulders [0, 5) and [5, 15), mid
   %        sun [15, 30), and high sun [30, 90]. The twilight edge reuses
   %        civil_twilight_deg's value so both passes share one boundary.
   %     min_bin_samples : 200, the overlap samples a season x elevation
   %        bin needs before the code uses its own ratio. A thinner bin
   %        inherits the season's single ratio. 200 hourly samples is a
   %        few weeks of a season's ~2200 hours. That is enough to
   %        stabilize a median ratio and still lets the narrow twilight
   %        bins qualify.
   ...
   % One struct, so a caller reads only the values it needs. Define the
