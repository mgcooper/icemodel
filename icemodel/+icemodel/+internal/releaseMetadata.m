function result = releaseMetadata(mode, kwargs)
   ...
   switch mode
      case "prepare"
         result = prepareRelease(cff_file, persisted_version, ...
            release_version, ...
...
function result = prepareRelease(cff_file, previous_version, version, ...
      date_released, ...
      preflight, validator)
   ...
   % Remove the prior release DOI, then update the two release scalars.
   % Invalid candidate metadata never replaces the live CFF.
   text = fileread(cff_file);
   text = removeVersionDoiIdentifier(text, previous_version);
...
function text = removeVersionDoiIdentifier(text, version)
   %REMOVEVERSIONDOIIDENTIFIER Remove the generated DOI for one release.

   escaped_version = regexptranslate('escape', char(version));
   description = ['The version DOI for IceModel v' escaped_version];
   description_pattern = ['(?m)^[ \t]+-[ \t]+description:[ \t]*' ...
      '["'']' description '["''][ \t]*\r?$'];
   entry_pattern = [description_pattern '\n' ...
      '[ \t]+type:[ \t]*doi[ \t]*\r?\n' ...
      '[ \t]+value:[ \t]*10\.5281/zenodo\.[0-9]+[ \t]*' ...
      '(?:\r?\n|$)'];
   descriptions = regexp(text, description_pattern, 'match');
   entries = regexp(text, entry_pattern, 'match');
   if numel(descriptions) ~= numel(entries) || numel(entries) > 1
      error('icemodel:internal:releaseMetadata:identifiersMalformed', ...
         'CFF has a malformed or duplicate version DOI entry for v%s', ...
         version)
   end
   if ~isempty(entries)
      text = regexprep(text, entry_pattern, '', 'once');
   end
end
