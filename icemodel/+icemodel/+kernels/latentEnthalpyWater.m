function [Le, Ls] = latentEnthalpyWater(T)
   %LATENTENTHALPYWATER Canonical Romps/Ambaum latent enthalpy reference.
   %
   ...
   %
   %  Temperature dependence: Le(T) and Ls(T) are linear in T, which follows
   %  the Kirchhoff relation dL/dT = cpv - cp_phase. The Rankine-Kirchhoff
   %  saturation vapor pressure formula es = a*exp(b/T)*T^c absorbs this
   %  temperature dependence into the coefficients a, b, c. Production code
   %  (icemodel.vapor.saturation_vapor_pressure, VAPORHEAT, etc.) therefore
   %  holds L constant at the reference value (Lv0 = 2.501e6,
   %  Ls0 = 2.834e6 J/kg). The Rankine-Kirchhoff exponents already carry the
   %  L(T) effect.
   %
   %  References:
...
function Le = latentHeatVaporization(T, E0v, Rv, cvv, cvl, T0)
   ...
   %
   % In icemodel production code (icemodel.vapor.initialize_vapor_model),
   % cp_liq and cp_ice refer to the measurement-consensus values at 0C
   % (4218, 2093), not the Ambaum fitting values (4220, 2097). The difference
   % is negligible.
   %
...
% Alternative forms of the same relations.
%
% Ambaum:
% L0 = E0v + Rv * T0;
