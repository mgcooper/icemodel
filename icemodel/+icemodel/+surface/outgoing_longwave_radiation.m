function [Qle, dQle_dTsfc] = outgoing_longwave_radiation(T_sfc, Qli)
   ...
   %
   %  [...] = icemodel.surface.outgoing_longwave_radiation(T_sfc, Qli)
   %
   % Computes outgoing longwave radiation in accordance with Kirchoff's Law,
   % where the upwelling longwave flux contains an emitted component, following
   % the Stefan-Boltzmann equation, and a reflected component, which is
   % proportional to the downwelling flux:
   %
   ...
   % Option to return the pure Stefan-Boltzmann form or the correct form.
   if nargin < 2
      Qle = - SB * emiss * T_sfc .^ 4;
   else
