function [T_sfc, ok] = solve_surface_energy_balance(T_sfc, tair, Qsi, ...
   ...
   switch seb_solver
      ...
      case 1 % Newton-Raphson - analytical derivative
         ...
         for iter = 1:maxiter
            ...
            [T_sfc, ok] = icemodel.surface.solve_surface_temperature( ...
               old, tair, Qsi, Qli, albedo, wspd, ppt, tppt, ...
               ea_atm, H_h, H_e, br_coefs, liqflag, chi, ...
               T_ice, k_eff, dz);

            if ~ok
               break
            end
            if abs(old - T_sfc) < tol ...
                  && abs(surface_residual(T_sfc)) < seb_tol
               ok_cpl = true;
               break
            end
         end
      ...
      case 2 % Complex-step - numerical derivative
         ...
         for iter = 1:maxiter
            old = T_sfc;
            [T_sfc, ok] = icemodel.numerics.complexstep( ...
               @surface_residual, old);
            if ~ok
               break
            end
            if abs(old - T_sfc) < tol ...
   ...
   if seb_solver == 0 || ~ok_cpl || abs(T_sfc - tair) > 20
      ...
      for iter = 1:maxiter
         old = T_sfc;
         [T_sfc, ~, ok] = icemodel.numerics.fsearchzero( ...
            @surface_residual, old, tair-20, tair+20, tair, tol);
         if ~ok
            break
         end
         if abs(old - T_sfc) < tol ...
   ...
   if ~ok && debug
      dumpSebSolveFailure(seb_solver, iter, T_sfc_old, T_sfc, tair, ...
   ...
   function f = surface_residual(T_sfc_local)
      f = icemodel.surface.surface_energy_balance_residual( ...
         T_sfc_local, tair, Qsi, Qli, albedo, wspd, ppt, tppt, ...
...
function dumpSebSolveFailure(solver, iter, T_sfc_old, T_sfc, tair, ...
   ...
   icemodel.surface.dump_turbulent_heat_flux_debug_state( ...
      "sebsolve_nonconvergence", T_sfc_old, T_sfc, tair, Qsi, Qli, ...
      albedo, wspd, ppt, tppt, psfc, ea_atm, ro_atm, cv_atm, nu_air, ...
      H_h, H_e, hv_atm, br_coefs, liqflag, chi, T_ice, k_eff, dz, ...
