function Qsi = terrain_adjusted_shortwave_radiation(Jday, xlat, ...
   ...
   cos_Z = max(0.0, cos_Z);
   sol_dec_eval = sol_dec + zeros(size(cos_Z));
   hr_angl_eval = hr_angl + zeros(size(cos_Z));
   ...
   sun_azimuth = zeros(size(cos_Z));
   valid_azimuth = sin_Z > 0.0 & cos_Z > 0.0;
   if any(valid_azimuth(:))
      sin_sun_azimuth = zeros(size(cos_Z));
      sin_sun_azimuth(valid_azimuth) = cos(sol_dec_eval(valid_azimuth)) ...
         .* sin(hr_angl_eval(valid_azimuth)) ./ sin_Z(valid_azimuth);
      sun_azimuth(valid_azimuth) = asin(max(-1.0, min(1.0, ...
         sin_sun_azimuth(valid_azimuth))));

      % Keep the azimuth referenced from the slope normal even when the sun
      % moves below the local slope horizon.
      morning = valid_azimuth & hr_angl_eval < 0.0 & hr_angl_eval < sun_azimuth;
      afternoon = valid_azimuth & hr_angl_eval > 0.0 & hr_angl_eval > sun_azimuth;
      sun_azimuth(morning) = -pi_value - sun_azimuth(morning);
      sun_azimuth(afternoon) = pi_value - sun_azimuth(afternoon);
   end
   ...
   slope_az_S0(slopeaz >= 180.0) = slopeaz(slopeaz >= 180.0) - 180.0;
