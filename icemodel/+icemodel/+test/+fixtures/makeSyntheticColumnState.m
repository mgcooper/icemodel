function state = makeSyntheticColumnState(workspace, smbmodel, kwargs)
   ...
   [ea_atm_all, ro_atm_all, cv_atm_all, nu_air_all, H_h_all, ...
      ...
      met.tair, met.wspd, met.rh, met.psfc);
   ...
   % The conservative recovery floor, resolved from its single source the
   % same way the production drivers resolve it.
   state.cpl_alpha_min = min(opts.cpl_alpha, ...
