function [baseline_type, baseline_tag, output_file, input_path, ...
   ...
   %     cases] = icemodel.test.helpers.prepareBaselineBuild(kind, ...
   ...
   arguments
      ...
      tier (1, :) string {icemodel.validators.mustBeTestTierName(tier)} = "full"
      smbmodel (1, :) string ...
         {icemodel.validators.mustBeTestSmbmodelSelector(smbmodel)} = "all"
   ...
   input_path = icemodel.getpath('input');
   output_path = icemodel.getpath('output');

   switch kind
      case "perf"
         cases = icemodel.test.helpers.getPerfCaseMatrix( ...
            tier=tier, smbmodel=smbmodel, solver=solver, simyear=simyear, ...
            baseline=baseline_tag, smoke_sites=smoke_sites, ...
