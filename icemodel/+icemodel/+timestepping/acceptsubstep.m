function [Ts, T, f_ice, f_liq, k_eff, dt_sum, dt_new, settings, diag] = ...
      acceptsubstep(Ts, T, f_ice, f_liq, k_eff, dt_sum, dt_new, TINY, ...
      settings, settings0, diag)
   %ACCEPTSUBSTEP Accept the substep: checkpoint state and time bookkeeping.
   %
   %  [Ts, T, f_ice, f_liq, k_eff, dt_sum, dt_new, settings, diag] = ...
   %     icemodel.timestepping.acceptsubstep( ...
   %     Ts, T, f_ice, f_liq, k_eff, dt_sum, dt_new, TINY, ...
   %     settings, settings0, diag)
   %
   % Accepting a substep is one operation with four parts:
   %   1. Checkpoint the accepted state (Ts, T, f_ice, f_liq, k_eff) for
