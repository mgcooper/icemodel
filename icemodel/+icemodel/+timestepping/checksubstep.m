function [T_sfc, T_ice, f_ice, f_liq, k_eff, substep, dt, all_ok, ...
      forced_advance, force_advance_streak_dt, settings, diag] = ...
      checksubstep(...
      T_sfc, T_ice, f_ice, f_liq, k_eff, xT_sfc, xT_ice, xf_ice, xf_liq, ...
      xk_eff, dt_sum, dt, timestep, numsteps, substep, ...
   ...
   %
   %  [T_sfc, T_ice, f_ice, f_liq, k_eff, substep, dt, ok, forced_advance, ...
   ...
   elseif inner_ok && allow_recovery_mode
      ...
      [T_sfc, T_ice, f_ice, f_liq, k_eff] = deal( ...
      ...
      [T_sfc_fail, T_ice_fail, f_ice_fail, f_liq_fail, k_eff_fail] = deal( ...
      ...
      [T_sfc, T_ice, f_ice, f_liq, k_eff, n_subfail, substep, dt] ...
         ...
         xT_sfc, xT_ice, xf_ice, xf_liq, xk_eff, settings.dt_full_step, ...
         substep, settings.maxsubstep, diag.n_failed_substeps, dt_sum);
      ...
      if settings.debug
         ...
         dumpMaxsubstepDebugState(T_sfc_fail, T_ice_fail, f_ice_fail, ...
            f_liq_fail, k_eff_fail, xT_sfc, xT_ice, xf_ice, xf_liq, xk_eff, ...
            dt_sum, dt, settings.dt_full_step, timestep, numsteps, substep, ...
            settings.maxsubstep, n_subfail, forced_advance, ...
            projected_force_advance_dt);
      end
   end
...
function dumpMaxsubstepDebugState(T_sfc_fail, T_ice_fail, f_ice_fail, ...
      f_liq_fail, k_eff_fail, xT_sfc, xT_ice, xf_ice, xf_liq, xk_eff, ...
      dt_sum, dt, dt_full_step, timestep, numsteps, substep, maxsubstep, ...
      n_subfail, forced_advance, projected_force_advance_dt)
   %DUMPMAXSUBSTEPDEBUGSTATE Save the failed and checkpoint states.
   %
   % Each field takes the name of the variable it stores. The force-advance
   % streak limit is one full step, so dt_full_step also records that limit.
   ...
   debug_state.projected_force_advance_dt = projected_force_advance_dt;
   debug_state.failed = struct('T_sfc', T_sfc_fail, 'T_ice', T_ice_fail, ...
      'f_ice', f_ice_fail, 'f_liq', f_liq_fail, 'k_eff', k_eff_fail);
   debug_state.checkpoint = struct('T_sfc', xT_sfc, 'T_ice', xT_ice, ...
