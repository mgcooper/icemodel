function [es, des_dT, d2es_dT2] = saturation_vapor_pressure(T, liqflag)
   ...
   %
   %  [ES, DES_DT] = icemodel.vapor.saturation_vapor_pressure(T, LIQFLAG) also
   %  computes the first temperature derivative:
   %
   %     d(es)/dT = es / T * (c - b / T)   [Pa K-1]
   %
   %  [ES, DES_DT, D2ES_DT2] = icemodel.vapor.saturation_vapor_pressure(T,
   %  LIQFLAG) also computes the second temperature derivative:
   %
   ...
   %  by icemodel.vapor.saturation_vapor_density.
   %
   % Notes
   %
   %  des_dT      = es     / T   * (c - b/T)
   ...
   %  icemodel.vapor.dew_point_temperature, icemodel.kernels.buckVaporModel
   %
   %#codegen
