   ...
   for channel = channels
      ...
      if any(missing_dark)
         findings(end + 1) = finding("error", ...
            ...
            'interval is below the -6 degree civil-night threshold'], ...
            nnz(missing_dark))); %#ok<AGROW>
      end
   end
end
...
function findings = precipitationRelationship(T, record)
   ...
   mismatch = abs(T.ppt(valid) - T.rainf(valid) - T.snowf(valid));
   scale = max(abs(T.ppt(valid)), 1e-12);
   if any(mismatch > max(1e-12, 1e-6 .* scale))
