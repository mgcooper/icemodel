function [summary, aligned, diagnostics, policy] = compareAblation( ...
   ...
   if any(gross_values < 0, 'all')
      error('icemodel:verification:compareAblation:negativeGross', ...
         'diagnostic gross fields must be nonnegative')
   end
...
function identities = closureIdentities(ledger, policy, Ls, Lv, ro_liq)
   ...
   % Preserve native substep/event flux gross totals while testing every output
   % row. The window normalizer uses its endpoint change, not storage or net
   % remesh gross, exactly as the accepted closure policy requires.
   p_s_q = ledger.mass_budget_phase_solid_gross_mwe;
   p_l_q = ledger.mass_budget_phase_liquid_gross_mwe;
   v_s_q = ledger.mass_budget_vapor_solid_gross_mwe;
   v_l_q = ledger.mass_budget_vapor_liquid_gross_mwe;
   r_s_q = ledger.mass_budget_remesh_solid_gross_mwe;
   r_l_q = ledger.mass_budget_remesh_liquid_gross_mwe;
   ...
   % changing their sum. The per-phase closures need its increments; a
   % cohort saved before the coupled mode carries no such channel, and
   % zero is what a run that never entered that path accumulated.
   x_s = additiveColumn(ledger, ...
      'mass_budget_vapor_redistribution_solid_mwe');
   x_l = additiveColumn(ledger, ...
      'mass_budget_vapor_redistribution_liquid_mwe');
   x_s_q = additiveColumn(ledger, ...
      'mass_budget_vapor_redistribution_solid_gross_mwe');
   x_l_q = additiveColumn(ledger, ...
   ...
   rows(4) = identityRow("total_storage", "mwe", ...
      ...
      p_s + p_l + v_s + v_l + r_s + r_l + x_s + x_l, ...
      p_s_q + p_l_q + v_s_q + v_l_q + r_s_q + r_l_q + x_s_q + x_l_q, ...
      policy, 1);
   ...
   %
   % A cohort saved under the retired schema recorded interior transport
   % inside the surface vapor channels and corrected this identity through
   % mass_budget_vapor_redistribution_j_m2. No production run wrote one,
   % because the coupled flag defaults off and no production cohort
   % enabled it, but such a cohort would
   % close neither the old identity nor this one, so reject a nonzero
   % retired channel rather than report a false residual.
   retired = 'mass_budget_vapor_redistribution_j_m2';
   if ismember(retired, ledger.Properties.VariableNames) ...
         && any(ledger.(retired) ~= 0)
      error('icemodel:verification:compareAblation:retiredCoupledSchema', ...
         ['the saved cohort carries nonzero %s, a coupled channel from ' ...
         'the retired schema; rerun the cohort'], retired)
   end
   ...
   o_l = ledger.mass_budget_merge_export_liquid_mwe;
   o_h = ledger.mass_budget_merge_export_enthalpy_j_m2;
   ...
   rows(6) = identityRow("remesh_solid", "mwe", ...
      ...
      + ledger.mass_budget_merge_export_solid_gross_mwe, policy, 1);
   rows(7) = identityRow("remesh_liquid", "mwe", ...
      sum(r_l), r_l, b_l - o_l, ...
      ledger.mass_budget_cloned_bottom_liquid_gross_mwe ...
      + ledger.mass_budget_merge_export_liquid_gross_mwe, policy, 1);
...
function row = identityRow(name, units, window_delta, step_delta, step_flux, ...
   ...
   normalization = max(policy.numerical.q_floor_mwe * scale, ...
      abs(window_delta) + sum(step_flux_gross));
   ...
   step_normalization = max(policy.numerical.q_floor_mwe * scale, ...
      abs(step_delta) + step_flux_gross);
...
function materiality = materialityDiagnostics(ledger, endpoint_deficit, ...
   ...
   rows = [ ...
      ...
      ledger.mass_budget_liquid_storage_gross_mwe, signal, policy); ...
      materialityRow("remesh_solid", ...
      ledger.mass_budget_remesh_solid_mwe, ...
      ledger.mass_budget_remesh_solid_gross_mwe, signal, policy); ...
      materialityRow("remesh_liquid", ...
      ledger.mass_budget_remesh_liquid_mwe, ...
      ledger.mass_budget_remesh_liquid_gross_mwe, signal, policy); ...
...
function row = materialityRow(channel, values, gross_values, signal, policy)
