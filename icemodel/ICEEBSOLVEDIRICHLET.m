function [Ts, T, f_ice, f_liq, k_eff, ok_seb, ok_ieb, ok, n_iters] = ...
      ICEEBSOLVEDIRICHLET(xTs, xT, xf_ice, xf_liq, dz, delz, fn, Sc, Sp, ...
      dt, JJ, k_liq, cv_ice, cv_liq, ro_ice, ro_liq, Ls, Lf, roLf, Tf, ...
      fcp, TL, TH, f_ell_min, f_ell_max, tair, swd, lwd, albedo, wspd, ...
      ppt, tppt, psfc, De, ea, chi, roL, scoef, liqflag, seb_solver, tol, ...
      maxiter, use_aitken, jumpmax, cpl_maxiter, cpl_Ts_tol, cpl_seb_tol, ...
      cpl_alpha, cpl_aitken, cpl_jumpmax, debug, ro_sfc, snow_depth, opts)
   %ICEEBSOLVEDIRICHLET Coupled surface/subsurface solve for solver=1.
   ...
   % physical diagnosed fluxes still apply the accepted physical-surface
   % contract downstream (for example via MELTTEMP-aware diagnostics).
   %
   %#codegen
   ...
   [Ts, ok_seb] = SEBSOLVE(xTs, tair, swd, lwd, albedo, wspd, ppt, tppt, ...
      psfc, De, ea, chi, roL, scoef, liqflag, xT, k_eff, dz, ro_sfc, ...
   ...
   for cpliter = 1:cpl_maxiter
      ...
      [Ts, ok_seb] = SEBSOLVE(Ts, tair, swd, lwd, albedo, wspd, ppt, tppt, ...
         psfc, De, ea, chi, roL, scoef, liqflag, T, k_eff, dz, ro_sfc, ...
         snow_depth, seb_solver, debug, opts);
      Ts_diag = Ts;
...
function dumpIceEbSolveDirichletFailure(reason, Ts, Ts_diag, Ts_old, T, ...
   ...
   debug_file = getenv('ICEMODEL_DEBUG_ICEEBSOLVE_DIRICHLET_FILE');
