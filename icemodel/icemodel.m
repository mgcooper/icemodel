      ...
      for timestep = 1:numsteps
         ...
         while dt_sum + TINY < dt_FULL_STEP
            ...
            if solver == 1 % Coupled Dirichlet surface-subsurface solve
               ...
               [T_sfc, T_ice, f_ice, f_liq, k_eff, ok_seb, ok_ieb, ok, ...
                  n_iters] = ICEEBSOLVEDIRICHLET(xT_sfc, xT_ice, xf_ice, ...
                  xf_liq, dz, delz, fn, Sc, Sp, dt, JJ, k_liq, cv_ice, ...
                  cv_liq, ro_ice, ro_liq, Ls, Lf, roLf, Tf, fcp, TL, TH, ...
                  f_ell_min, f_ell_max, tair(metstep), swd(metstep), ...
                  lwd(metstep), albedo(metstep), wspd(metstep), ...
                  ppt(metstep), tppt(metstep), psfc(metstep), De(metstep), ...
                  ea, chi, roL, scoef, liqflag, seb_solver, tol, maxiter, ...
                  use_aitken, jumpmax, cpl_maxiter, cpl_Ts_tol, ...
