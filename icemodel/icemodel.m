   ...
   for thisyear = 1:numyears
      ...
      for iter = 1:maxiter
         ...
         while dt_sum + TINY < dt_FULL_STEP
            ...
            % ICE ENERGY BALANCE
            if bc == 3
               [Ts, Fc, Fp, T, f_ice, f_liq, k_eff, OK, N, a1] = ICEEBSOLVE( ...
                  T, f_ice, f_liq, dz, delz, fn, Sc, dt, JJ, Ts, k_liq, ...
                  cv_ice, cv_liq, ro_ice, ro_liq, Ls, Lf, roLf, Rv, Tf, fcp, ...
                  TL, TH, f_ell_min, f_ell_max, bc, maxcpliter, omega, tol, ...
                  tair(metiter), swd(metiter), lwd(metiter), albedo(metiter), ...
                  wspd(metiter), psfc(metiter), De(metiter), ea, cv_air, ...
                  emiss, SB, roL, scoef, chi, liqflag);
