classdef ColumnKernelPerfTest < matlab.perftest.TestCase
   ...
   methods (Test)
      function testUpdateState(testCase)
         ...
         s = testCase.ice;
         % icemodel.column.updatestate is extremely fast, so each timed sample needs a
      ...
      function testIceEbSolve(testCase)
        ...
        [Ts, T, f_ice, f_liq, k_eff, ok_ieb] = ...
            ...
            s.ea_atm, s.br_coefs, s.roL, s.liqflag, s.chi, 3, s.tol, ...
            s.maxiter, s.alpha, ...
         ...
         while testCase.keepMeasuring
            for n = 1:batch_size
               [Ts, T, f_ice, f_liq, k_eff, ok_ieb] = ...
                  ...
                  3, s.tol, s.maxiter, s.alpha, s.use_aitken, ...
