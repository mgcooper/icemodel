classdef SebKernelPerfTest < matlab.perftest.TestCase
   ...
   methods (TestMethodSetup)
      function generateTestData(testCase)
         ...
         testCase.ea_atm = icemodel.vapor.saturation_vapor_pressure(testCase.Ta, testCase.liqflag) ...
            .* testCase.rh / 100;
   ...
   methods (Test)
      function testSolver(testCase, seb_solver)
         % Benchmark one surface-energy-balance root-finder variant on a fixed surface
         % state so only solver-path overhead changes between cases.
         batch_size = sebBenchmarkBatchSize(seb_solver);

         % Benchmark only the solve_surface_energy_balance call for each available root finder.
         ...
         [Ts, ok] = icemodel.surface.solve_surface_energy_balance( ...
            ...
            testCase.br_coefs, testCase.roL, testCase.liqflag, testCase.chi, ...
            testCase.T, testCase.k_eff, testCase.dz, ...
            [], [], opts_sv);
         ...
         while testCase.keepMeasuring
            for n = 1:batch_size
               [Ts, ok] = icemodel.surface.solve_surface_energy_balance( ...
                  ...
                  testCase.De, testCase.ea_atm, testCase.br_coefs, testCase.roL, ...
                  testCase.liqflag, testCase.chi, testCase.T, ...
                  testCase.k_eff, testCase.dz, [], [], opts_sv);
            end

            if ~ok || ~isfinite(Ts)
               error('solve_surface_energy_balance benchmark failed for seb_solver=%d', ...
                  seb_solver)
            end
         end
      end
