function results = calibrate_mo_roughness(z0_ice_values, make_plots, case_idx)
   %CALIBRATE_MO_ROUGHNESS Sweep z0_ice on real KANM/KANL cases (MO scheme).
   %
   %  results = calibrate_mo_roughness()
   %  results = calibrate_mo_roughness([0.001 0.003 0.01])
   %  results = calibrate_mo_roughness([], true)
   %  results = calibrate_mo_roughness([], false, 1:2)
   %
   % Sweeps the bare-ice momentum roughness length z0_ice over a user-supplied
   % grid of values and evaluates goodness-of-fit against KANM/KANL observations
   % for each value. Uses the Monin-Obukhov scheme exclusively so that the
   % sensitivity is isolated to this single parameter. The current cases are:
   %   - KANM 2016
