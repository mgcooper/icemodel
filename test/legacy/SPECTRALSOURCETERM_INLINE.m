function [Sc, chi] = SPECTRALSOURCETERM_INLINE(Qsi, albedo, I0, dz_spect, ...
      tau_N, tau_S, solar_dwavel, dz_therm, ro_sno, z_nodes_therm, ...
      z_nodes_spect)
   %SPECTRALSOURCETERM_INLINE Historical inlined spectral source-term path.
   %
   %  [Sc, chi] = SPECTRALSOURCETERM_INLINE(Qsi, albedo, I0, dz_spect, ...
   %     tau_N, tau_S, solar_dwavel, dz_therm, ro_sno, z_nodes_therm, ...
   %     z_nodes_spect)
   %
   % This preserves the pre-refactor inlined source-term implementation so the
   % spectral study can compare code organization and lookup changes against a
   % historical reference while reusing the shared precomputed geometry.
   %
   %#codegen

   if Qsi < 1e-3
      Sc = zeros(size(dz_therm));
      chi = 1.0;
      return
   end

   % Transform the mass density to the spectral grid.
   ro_sno_spect = interp1(z_nodes_therm, ro_sno, z_nodes_spect, ...
      'nearest', 'extrap');
   ro_sno_spect = max(ro_sno_spect, 300.0);

   % Compute the downward bulk extinction coefficient and pad the lower
   % boundaries expected by the two-stream solve.
   bulkcoefs = -log((sum(solar_dwavel .* exp(tau_S .* ...
      ro_sno_spect), 2)) ./ (sum(solar_dwavel .* exp( ...
      tau_N .* ro_sno_spect), 2))) / dz_spect(1);
   bulkcoefs = [bulkcoefs; bulkcoefs(end); bulkcoefs(end)];

   % Compute the two-stream coefficients directly from the extinction profile.
   a = ((1.0 - albedo) / (1.0 + albedo)) * bulkcoefs;
   r = (2.0 * albedo / (1.0 - albedo ^ 2)) * bulkcoefs;

   % Solve the inlined two-stream system.
   M = numel(dz_spect) + 2;
   e = zeros(M - 1, 1);
   f = zeros(M - 1, 1);
   g = zeros(M - 1, 1);
   b = zeros(M - 1, 1);

   alfa = 1.0 / (a(1) + r(1));
   e(1) = 0.0;
   f(1) = 1.0;
   g(1) = -alfa / (dz_spect(1) + alfa);
   b(1) = r(1) * I0 * dz_spect(1) * alfa / (dz_spect(1) + alfa);

   deltaz = [dz_spect(2:end); dz_spect(end)];
   e(2:M - 1) = 1.0 + (r(3:M) - r(1:M - 2)) ./ (4.0 * r(2:M - 1));
   f(2:M - 1) = deltaz ./ (2.0 * r(2:M - 1)) .* ...
      (a(2:M - 1) .* (r(3:M) - r(1:M - 2)) ...
      - r(2:M - 1) .* (a(3:M) - a(1:M - 2))) ...
      - (2.0 + deltaz .^ 2 .* bulkcoefs(2:M - 1) .^ 2);
   g(2:M - 1) = 1.0 - (r(3:M) - r(1:M - 2)) ./ (4.0 * r(2:M - 1));
   b(2:M - 1) = 0.0;
   g(M - 1) = 0.0;
   b(M - 1) = 0.0;

   up = zeros(M - 1, 1);
   for k = 2:numel(f)
      f(k) = f(k) - e(k) / f(k - 1) * g(k - 1);
      b(k) = b(k) - e(k) / f(k - 1) * b(k - 1);
   end
   up(M - 1) = b(M - 1) / f(M - 1);
   for k = numel(f) - 1:-1:1
      up(k) = (b(k) - g(k) * up(k + 1)) / f(k);
   end
   up = [up(1:M - 1); 0];

   % Reconstruct and smooth the downward and upward flux profiles.
   dn = [I0; ...
      (a(2:M - 1) + r(2:M - 1)) ./ r(2:M - 1) .* up(2:M - 1) ...
      - (up(3:M) - up(1:M - 2)) ./ (2.0 * deltaz .* r(2:M - 1)); ...
      (a(M) + r(M)) / r(M) * up(M) ...
      - (up(M) - up(M - 1)) / (deltaz(M - 2) * r(M))];
   [up, dn] = smoothFluxProfiles(up, dn, M);

   % Reconstruct the net flux from the staggered up/down fluxes.
   M = numel(dz_spect) + 2;
   Q = [min(-I0 * (1 - albedo), up(1) - dn(1)); ...
      (up(2:M - 1) + up(3:M)) / 2.0 - (dn(2:M - 1) + dn(3:M)) / 2.0];
   [Sc, chi] = inlineNetFluxToSourceTerm(Qsi, albedo, I0, Q, dz_spect, ...
      dz_therm);
end

function [up, dn] = smoothFluxProfiles(up, dn, M)
   %SMOOTHFLUXPROFILES Reproduce the historical smoothing pass.

   dntmp = (dn(2:M - 1) + 0.5 * (dn(1:M - 2) + dn(3:M))) / 2.0;
   uptmp = (up(2:M - 1) + 0.5 * (up(1:M - 2) + up(3:M))) / 2.0;

   dntmp = [(dn(2) + 0.5 * dn(1)) / 1.5; dntmp];
   uptmp = [(up(2) + 0.5 * up(1)) / 1.5; uptmp];
   dntmp = [dntmp; (dn(M - 1) + 0.5 * dn(M)) / 1.5];
   uptmp = [uptmp; (up(M - 1) + 0.5 * up(M)) / 1.5];

   dn = [dn(1); dntmp(2:M - 1); dn(end)];
   up = [up(1); uptmp(2:M - 1); up(end)];

   dntmp = (dn(2:M - 1) + 0.5 * (dn(1:M - 2) + dn(3:M))) / 2.0;
   uptmp = (up(2:M - 1) + 0.5 * (up(1:M - 2) + up(3:M))) / 2.0;

   dntmp = [(dn(2) + 0.5 * dn(1)) / 1.5; dntmp];
   uptmp = [(up(2) + 0.5 * up(1)) / 1.5; uptmp];
   dntmp = [dntmp; (dn(M - 1) + 0.5 * dn(M)) / 1.5];
   uptmp = [uptmp; (up(M - 1) + 0.5 * up(M)) / 1.5];

   dn = [dn(1); dntmp(2:M - 1); dn(end)];
   up = [up(1); uptmp(2:M - 1); up(end)];
end

function [Sc, chi] = inlineNetFluxToSourceTerm(Qsi, albedo, I0, Q, ...
      dz_spect, dz_therm)
   %INLINENETFLUXTOSOURCETERM Local copy of the production source-term collapse.

   dQ_spect = Q(1:end - 1) - Q(2:end);
   n_spect_per_therm = dz_therm(1) / dz_spect(1);
   dQ_therm = transpose(sum(reshape(dQ_spect, n_spect_per_therm, []), 1));
   dQ_therm = [dQ_therm; zeros((sum(dz_therm) - sum(dz_spect)) ...
      / dz_therm(1), 1)];

   if albedo > 0.65
      chi = 0.9;
   else
      chi = dQ_therm(1) / sum(dQ_therm);
   end

   Sc = -((1.0 - chi) * Qsi / I0) * dQ_therm ./ dz_therm;
end
