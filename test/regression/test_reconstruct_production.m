...
function test_current_promice_filled_files_record_engine_version(testCase)
   % Current release forcing must name the engine that produced it. The
   % stamp is recorded, not compared with icemodel.internal.version(),
   % because a shipped file predates the release that ships it.
   % The shipped files exist only for a release whose registered forcing is
   % promice_filled. Assume that, so a release without it filters this test
   % instead of reporting a pass that asserted nothing.
   release_version = "v" + string(icemodel.internal.version());
   policy = icemodel.test.helpers.formalBaselinePolicy(release_version);
   testCase.assumeEqual(policy.forcing, "promice_filled");

   root = icemodel.verification.setup.fixtureDataRoot(release_version);
   [~, selection] = icemodel.verification.setup.fixtureFileList( ...
      capabilities="formal-core", root=root);
   paths = string({selection.files.path});
   paths = paths(contains(paths, "/promice_filled/") ...
      & endsWith(paths, ".mat"));
   testCase.assertNotEmpty(paths);
   for relative_path = paths
      saved = load(fullfile(root, relative_path), ...
         'artifact_metadata', 'met');
      artifact_version = ...
         string(saved.artifact_metadata.gapfill_engine_version);
      met_version = string( ...
         saved.met.Properties.UserData.gapfill_engine_version);
      testCase.verifyTrue(isscalar(artifact_version) ...
         && strlength(artifact_version) > 0);
      testCase.verifyTrue(isscalar(met_version) ...
         && strlength(met_version) > 0);
      % Both stamps describe one file, so they must agree with each other.
      testCase.verifyEqual(met_version, artifact_version);
   end
end
...
function test_fill_station_keeps_hourly_wind_above_forcing_floor(testCase)
   ...
   out_dir = fullfile(root, 'met', 'promice_filled');
   qa_dir = fullfile(root, 'preview', 'qa', 'gapfill');
   ...
   testCase.verifyEqual(string(verified.report_inputs_file), ...
      string(fullfile(qa_dir, 'plans', 'wind-report-inputs.json')));
   ...
   refreshReportInputArtifact(verified.report_inputs_file, ...
      "filled", root);
   testCase.verifyError(@() ...
