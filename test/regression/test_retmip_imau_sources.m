...
function setupOnce(testCase)
   % Install the canonical verification path once for the whole suite. Per-
   % test setup previously called bootstrapTestEnvironment before every test,
   % which re-ran the full path scan 138 times.
   [~, ~, ~, ~, cleanup] = icemodel.test.helpers.bootstrapTestEnvironment();
   testCase.TestData.cleanup = cleanup;
end

function teardownOnce(testCase)
   % Release the bootstrap cleanup handle after the whole suite finishes.
   testCase.TestData.cleanup = [];
end

function setup(testCase)
   % Allocate an isolated source cache for each test.
...
function test_prepare_case_root_selects_only_missing_artifacts(testCase)
   ...
   % captureExpectedWarning already asserts the warning id, so the manual
   % lastwarn read/compare that used to follow this call is now redundant.
   selected = icemodel.test.helpers.captureExpectedWarning(testCase, ...
      @() icemodel.verification.setup.prepareCaseRoot( ...
   ...
   % overwrite=true replaces both fixed artifacts and fires the same warning
   % id once per call, regardless of how many artifacts it replaces.
   overwrite_call = @() icemodel.verification.setup.prepareCaseRoot( ...
      case_root, true, ["observations.mat", "reference.mat"]);
   overwrite_selected = icemodel.test.helpers.captureExpectedWarning( ...
      testCase, overwrite_call, ...
      'icemodel:verification:prepareCaseRoot:overwrite');
...
function test_prepare_case_root_gates_fixed_artifact_identity(testCase)
   ...
   for k = 1:numel(fields)
      ...
      % captureExpectedWarning asserts the id itself, replacing the manual
      % lastwarn read/compare that used to follow this call.
      changed = requested;
      name = char(fields(k));
...
function test_retmip_rcm_only_replacement_strips_prior_runtime(testCase)
   ...
   % This call reuses the "mar" forcing already staged by the initial call, so
   % it fires the manifest, userdata, and met overwrite warnings in sequence;
   % lastwarn only sees the last one, writemet:overwrite.
   replaced = icemodel.test.helpers.captureExpectedWarning(testCase, ...
      @() icemodel.verification.setup.importRetmip( ...
      missing_cache, case_ids="dye2", output_root=root, ...
      forcing_sources="mar", build_observations=false, build_forcing=true, ...
...
function test_retmip_native_only_refresh_updates_native_metadata(testCase)
   ...
   % The prior manifest carried "mar" forcing_sources; this native-only
   % refresh replaces the family manifest but writes distinct retmip-native
   % userdata/met filenames, so only the manifest overwrite warning fires.
   refreshed = icemodel.test.helpers.captureExpectedWarning(testCase, ...
      @() icemodel.verification.setup.importRetmip( ...
      cache, case_ids="dye2", output_root=root, ...
      forcing_sources="retmip", build_observations=false, ...
...
function test_fetch_gcnet_empty_station_selection_uses_defaults(testCase)
   ...
   % Strict failure prints a DOI guidance banner; capture it under evalc.
   evalc(['verifyMissingGcnetSources(testCase, testCase.TestData.cache, ' ...
      'strings(1, 0));']);
end
...
function test_fetch_gcnet_rejects_partial_and_ambiguous_matches(testCase)
   ...
   % Strict failure prints the same DOI guidance banner; capture it under evalc.
   evalc('verifyMissingGcnetSources(testCase, cache, "DYE_2");');
end
...
function test_retmip_import_stages_protocol_bundle(testCase)
   ...
   % No forcing rebuild runs here, so only prepareCaseRoot's overwrite id fires.
...
function test_retmip_protocol_refresh_preserves_native_runtime_leg(testCase)
   ...
   % hidden native-source probe or rewrite. No forcing rebuild runs, so only
   % prepareCaseRoot's overwrite id fires.
   second = icemodel.test.helpers.captureExpectedWarning(testCase, ...
      @() icemodel.verification.setup.importRetmip(alternate_retmip_cache, ...
...
function test_retmip_protocol_refresh_rejects_changed_native_identity(testCase)
   ...
   for k = 1:numel(fields)
      ...
      % No forcing rebuild runs on this branch, so the loop fires only the
      % prepareCaseRoot overwrite id, once per iteration.
      refreshed = icemodel.test.helpers.captureExpectedWarning(testCase, ...
...
function test_imau_import_dry_run_manifest(testCase)
   ...
   % Explicit dry-run windows must preview the same case period used by staging.
   windowed = icemodel.verification.setup.importImau( ...
      missing_imau, case_ids="S21", dry_run=true, ...
      startdate="2014-01-01", enddate="2014-12-31");
   testCase.verifyEqual(string(windowed.cases.period.start), ...
      "2014-01-01 00:00:00");
   testCase.verifyEqual(string(windowed.cases.period.end), ...
      "2014-12-31 00:00:00");
...
function test_imau_observation_refresh_preserves_native_artifacts(testCase)
   ...
   % No forcing rebuild runs here, so only prepareCaseRoot's overwrite id fires.
   refreshed = icemodel.test.helpers.captureExpectedWarning(testCase, ...
      @() icemodel.verification.setup.importImau(alternate_cache, ...
      case_ids="S21", evaluation_data_root=eval_root, ...
      input_data_root=input_root, forcing_sources=strings(1, 0), ...
...
function test_imau_observation_refresh_rejects_native_identity_conflicts(testCase)
   ...
   for variant = ["source", "point"]
      ...
      % No forcing rebuild runs on this branch, so each loop pass fires only
      % the prepareCaseRoot overwrite id.
      refreshed = icemodel.test.helpers.captureExpectedWarning(testCase, ...
         @() icemodel.verification.setup.importImau(cache, ...
         case_ids="S21", evaluation_data_root=eval_root, ...
         input_data_root=input_root, forcing_sources=strings(1, 0), ...
...
function test_imau_import_stages_native_hourly_sources(testCase)
   ...
   % Native input files are already current, so only the fixed observation
   % bundle rewrite fires prepareCaseRoot's overwrite id.
   icemodel.test.helpers.captureExpectedWarning(testCase, ...
      @() icemodel.verification.setup.importImau(cache, case_ids="S21", ...
      evaluation_data_root=eval_root, input_data_root=input_root, ...
      forcing_sources="imau", build_forcing=true, overwrite=false), ...
...
function test_imau_strict_native_only_refresh_does_not_require_daily_qa(testCase)
   ...
   % The prior manifest carried "mar" forcing_sources; this native-only
   % refresh writes distinct imau-native userdata/met filenames, so only the
   % manifest overwrite warning fires.
   refreshed = icemodel.test.helpers.captureExpectedWarning(testCase, ...
      @() icemodel.verification.setup.importImau( ...
...
function test_imau_import_stages_rcm_fixtures(testCase)
   ...
   % This reuses the "mar" forcing already staged by the initial call, so it
   % fires the userdata then met overwrite warnings in sequence; lastwarn only
   % sees the last one, writemet:overwrite.
...
function test_retmip_partial_rcm_refresh_only_updates_requested_case(testCase)
   ...
   % "fa" already has a staged observation bundle and gets its first forcing
   % build here, so only prepareCaseRoot's overwrite id fires.
   returned = icemodel.test.helpers.captureExpectedWarning(testCase, ...
      @() icemodel.verification.setup.importRetmip(retmip_cache, ...
...
function test_import_research_sites_stages_rcm_fixtures(testCase)
   ...
   % A MAR-only update must not erase omitted existing MERRA/RACMO legs. This
   % reuses the "mar" forcing already staged by the initial call, so it fires
   % the userdata then met overwrite warnings in sequence; lastwarn only sees
   % the last one, writemet:overwrite.
...
function test_import_sumup_rcm_only_reuses_observations_without_source_cache( ...
   ...
   % This reuses the "mar" forcing already staged by the initial call and
   % also replaces the family manifest, so it fires the manifest, userdata,
   % and met overwrite warnings in sequence; lastwarn only sees the last one,
   % writemet:overwrite.
   refreshed = icemodel.test.helpers.captureExpectedWarning(testCase, ...
      @() icemodel.verification.setup.importSumup(absent_cache, ...
      points=point, case_ids="humphrey", output_root=root, ...
      startdate="2010-07-01", enddate="2012-04-30", ...
      forcing_sources="mar", build_observations=false, ...
...
function verifyMissingGcnetSources(testCase, cache_dir, stations)
   %VERIFYMISSINGGCNETSOURCES Check the strict missing-source error.
   call = @() icemodel.verification.setup.fetchGcnet( ...
      cache_dir=cache_dir, stations=stations, products="surface", ...
      strict=true, silent=true);
   testCase.verifyError(call, ...
