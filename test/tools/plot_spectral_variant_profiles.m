function report = plot_spectral_variant_profiles(kwargs)
   ...
   arguments
      ...
      kwargs.solver (1, 1) double ...
         {icemodel.validators.mustBeSolverFilter(kwargs.solver)} ...
         = 3
...
function profiles = buildProfiles(s)
   ...
   k_lookup = icemodel.radiation.make_bulk_extinction_lookup( ...
   ...
   bulk_functions = icemodel.radiation.bulk_extinction_coefficients( ...
      s.dz_spect, ro_sno_spect, s.tau_N, s.tau_S, s.solar_dwavel);
   bulk_lookup = icemodel.radiation.bulk_extinction_coefficients(s.dz_spect, ...
      ro_sno_spect, s.tau_N, s.tau_S, s.solar_dwavel, k_lookup);

   % Solve the two-stream system for the exact and lookup bulk coefficients.
   Q_functions = icemodel.radiation.solvetwostream( ...
      s.I0, s.albedo, bulk_functions, s.z_edges_spect);
   Q_lookup = icemodel.radiation.solvetwostream( ...
   ...
   [Sc_functions, chi_functions] = icemodel.column.shortwave_source_term( ...
      ...
      s.dz, s.z_nodes, s.z_nodes_spect, s.z_edges_spect, ro_sno, k_lookup_empty);

   [Sc_lookup, chi_lookup] = icemodel.column.shortwave_source_term(s.swd, ...
      s.albedo, s.I0, s.dz_spect, s.tau_N, s.tau_S, s.solar_dwavel, s.dz, ...
      s.z_nodes, s.z_nodes_spect, s.z_edges_spect, ro_sno, k_lookup);
