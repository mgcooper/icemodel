...
function report = summarizeSpectralKernelPerf(simyear)
   ...
   k_lookup = icemodel.makeBulkExtCoefsLookup(state.dz_spect, ...
   ...
   bulk_exact_s = timeit(@() repeatCall(@() icemodel.radiation.bulk_extinction_coefficients(state.dz_spect, ...
      ro_sno_spect, state.tau_N, state.tau_S, state.solar_dwavel), 32)) / 32;

   bulk_lookup_s = timeit(@() repeatCall(@() icemodel.radiation.bulk_extinction_coefficients_lookup( ...
      ro_sno_spect, k_lookup), 256)) / 256;
   ...
   functions_s = timeit(@() repeatCall(@() icemodel.column.shortwave_source_term( ...
      state.swd, state.albedo, state.I0, state.dz_spect, state.tau_N, ...
      state.tau_S, state.solar_dwavel, state.dz, ro_sno, state.z_nodes, ...
      state.z_nodes_spect, state.z_edges_spect, k_lookup_empty), 8)) / 8;

   lookup_s = timeit(@() repeatCall(@() icemodel.column.shortwave_source_term( ...
      state.swd, state.albedo, state.I0, state.dz_spect, state.tau_N, ...
      state.tau_S, state.solar_dwavel, state.dz, ro_sno, state.z_nodes, ...
      state.z_nodes_spect, state.z_edges_spect, k_lookup), 8)) / 8;

   % Compute exact and approximate agreement metrics alongside the timings.
   bulk_exact = icemodel.radiation.bulk_extinction_coefficients(state.dz_spect, ro_sno_spect, state.tau_N, ...
      state.tau_S, state.solar_dwavel);

   bulk_lookup = icemodel.radiation.bulk_extinction_coefficients_lookup(ro_sno_spect, k_lookup);
   ...
   [Sc_functions, chi_functions] = icemodel.column.shortwave_source_term( ...
      state.swd, state.albedo, state.I0, state.dz_spect, state.tau_N, ...
      state.tau_S, state.solar_dwavel, state.dz, ro_sno, state.z_nodes, ...
      state.z_nodes_spect, state.z_edges_spect, k_lookup_empty);

   [Sc_lookup, chi_lookup] = icemodel.column.shortwave_source_term( ...
      state.swd, state.albedo, state.I0, state.dz_spect, state.tau_N, ...
      state.tau_S, state.solar_dwavel, state.dz, ro_sno, state.z_nodes, ...
      state.z_nodes_spect, state.z_edges_spect, k_lookup);
