...
function report = summarizeSpectralKernelPerf(simyear)
   ...
   bulk_lookup_s = timeit(@() repeatCall(@() BULKEXTCOEFSLOOKUP( ...
   ...
   functions_s = timeit(@() repeatCall(@() SPECTRALSOURCETERM( ...
      state.swd, state.albedo, state.I0, state.dz_spect, state.tau_N, ...
      state.tau_S, state.solar_dwavel, state.dz, ro_sno, state.z_nodes, ...
      state.z_nodes_spect, state.z_edges_spect, k_lookup_empty), 8)) / 8;

   lookup_s = timeit(@() repeatCall(@() SPECTRALSOURCETERM( ...
      state.swd, state.albedo, state.I0, state.dz_spect, state.tau_N, ...
      state.tau_S, state.solar_dwavel, state.dz, ro_sno, state.z_nodes, ...
   ...
   bulk_exact = BULKEXTCOEFS(state.dz_spect, ro_sno_spect, state.tau_N, ...
   ...
   [Sc_functions, chi_functions] = SPECTRALSOURCETERM( ...
      state.swd, state.albedo, state.I0, state.dz_spect, state.tau_N, ...
      state.tau_S, state.solar_dwavel, state.dz, ro_sno, state.z_nodes, ...
      state.z_nodes_spect, state.z_edges_spect, k_lookup_empty);

   [Sc_lookup, chi_lookup] = SPECTRALSOURCETERM( ...
      state.swd, state.albedo, state.I0, state.dz_spect, state.tau_N, ...
      state.tau_S, state.solar_dwavel, state.dz, ro_sno, state.z_nodes, ...
