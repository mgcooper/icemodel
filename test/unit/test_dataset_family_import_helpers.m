...
function test_pairedWindow_normalizes_optional_utc_bounds(testCase)
   ...
   [t1, t2, enabled] = icemodel.pairedWindow( ...
   ...
   [~, ~, enabled] = icemodel.pairedWindow(nat, nat);
   testCase.verifyFalse(enabled);
   ...
   [t1, t2, enabled] = icemodel.pairedWindow(ny_start, ny_end);
   testCase.verifyTrue(enabled);
   testCase.verifyEqual(t1.TimeZone, 'UTC');
...
function test_pairedWindow_rejects_half_reversed_and_vector_bounds(testCase)
   ...
   testCase.verifyError(@() ...
      icemodel.pairedWindow("2012-01-01", ""), ...
      error_id);
   testCase.verifyError(@() ...
      icemodel.pairedWindow("", "2012-01-02"), ...
      error_id);
   testCase.verifyError(@() ...
      icemodel.pairedWindow( ...
   ...
   testCase.verifyError(@() ...
      icemodel.pairedWindow("not-a-date", "still-not-a-date"), ...
      error_id);
end
...
function test_runDatasetFamilyImport_partial_refresh_preserves_omitted_rcm(testCase)
   ...
   state = helperState();
   % 2013 has zero overlap with the case's staged 2012 period, so the ordinary
   % (non-overwrite_family) manifest patch hits writeFamilyManifestMerge's
   % disjoint-coverage guard, not the RCM cache-reuse warning.
...
function test_runDatasetFamilyImport_overwrite_discards_existing_rcm(testCase)
   ...
   state = helperState();
   % overwrite_family=true against an existing manifest that drops the mar
   % leg trips writeFamilyManifestMerge's overwriteFamily warning.
   manifest = icemodel.test.helpers.captureExpectedWarning(testCase, ...
      @() icemodel.verification.setup.runDatasetFamilyImport( ...
      state, alive, dataset_family="helper", manifest_file=manifest_file, ...
      requested_ids="case1", source_url="test", source_version="v2", ...
...
function test_stageRcmForcing_manifest_preserves_prior_data_on_partial_cache(testCase)
   ...
   manifest = jsondecode(fileread(manifest_file));
   % This met-only rediscovery path stages quietly (verified: lastwarn is
   % empty after the call), so no expected-warning capture is needed.
   returned = icemodel.verification.setup.stageRcmForcing( ...
      obs_manifest=manifest, manifest_file=manifest_file, ...
      forcing_sources="mar", ...
      met_outdir=met_root, userdata_outdir=data_root, ...
...
function test_stageRcmForcing_bad_data_only_cache_does_not_skip_rebuild(testCase)
   ...
   % The saved 'notData' variable makes deriveExistingMetFromData's load('Data')
   % raise MATLAB's own variableNotFound warning before the derive attempt
   % fails and the code falls back to the (also-failing) raw rebuild.
   colocation = icemodel.test.helpers.captureExpectedWarning(testCase, ...
      @() icemodel.verification.setup.stageRcmForcing( ...
      [67, -48], legspec=legspec, forcing_sources="mar", ...
      userdata_outdir=data_root, method="nearest", ...
...
function test_stageDatasetFamilyCases_skips_label_callback_missing_data(testCase)
   ...
   % captureExpectedWarning returns one output only; this site needs
   % [~, alive, skipped], so route the call through evalc directly and check
   % lastwarn, mirroring the multi-output idiom used elsewhere in this file.
   lastwarn('');
   evalc(['[~, alive, skipped] = ' ...
   ...
   [~, id] = lastwarn();
   testCase.verifyEqual(id, 'test:skip');
