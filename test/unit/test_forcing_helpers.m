...
function test_writemet_rejects_identity_collision_and_scopes_pruning(testCase)
   ...
   % path, preserving the established destructive boundary. The overwrite
   % warning is expected here, so capture it to keep its text out of the
   % suite log while still checking the warning id.
   repaired = icemodel.test.helpers.captureExpectedWarning(testCase, ...
