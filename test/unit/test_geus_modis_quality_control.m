...
function test_repair_masks_sentinel_dry_run_write_and_idempotence(testCase)
   % The bounded repair must classify a nominally current 999-bearing artifact,
   % leave it untouched in dry-run mode, write only normalized data on request,
   % and become source-light and unchanged on the next pass.
   eval_root = fullfile(testCase.TestData.tmp, "repair-eval");
   input_root = fullfile(testCase.TestData.tmp, "repair-input");
   modis_dir = fullfile(testCase.TestData.tmp, "repair-source");
   relative = "racmo2.3p3/site_racmo2.3p3_20120101_20120103.mat";
   data_file = fullfile(input_root, "userdata", relative);
   [~, location] = writeTinyGeusModis( ...
      modis_dir, 2012, [0.5, 999, 0.7]);
   writeRepairManifest(eval_root, relative, location);
   writeSentinelArtifact(data_file);

   dry = icemodel.verification.setup.repairRcmArtifactMetadata( ...
      input_root, eval_root=eval_root, dataset_family="promice", ...
      modis_dir=modis_dir, dry_run=true);
   unchanged = load(data_file, 'Data');
   testCase.verifyEqual(string(dry.records.status), "would_repair");
   testCase.verifyEqual(unchanged.Data.modis, 999 * ones(3, 1));

   written = icemodel.verification.setup.repairRcmArtifactMetadata( ...
      input_root, eval_root=eval_root, dataset_family="promice", ...
      modis_dir=modis_dir, dry_run=false);
   repaired = load(data_file, 'Data', 'artifact_metadata');
   finite = repaired.Data.modis(isfinite(repaired.Data.modis));
   testCase.verifyEqual(string(written.records.status), "repaired");
   testCase.verifyEqual(repaired.Data.modis, [0.5; NaN; 0.7]);
   testCase.verifyTrue(all(finite >= 0 & finite <= 1));
   testCase.verifyEqual(double( ...
      repaired.artifact_metadata.modis_coverage_years(:))', 2012);
...
function test_repair_applies_racmo_ppt_qc_and_preserves_provenance(testCase)
   % Existing exact RACMO references use the builder's nonnegative invariant;
   % the second pass must retain the original negative-input provenance.
   eval_root = fullfile(testCase.TestData.tmp, "racmo-repair-eval");
   input_root = fullfile(testCase.TestData.tmp, "racmo-repair-input");
   relative = "racmo2.3p3/site_racmo2.3p3_20120101_20120103.mat";
   data_file = fullfile(input_root, "userdata", relative);
   writeRepairManifest(eval_root, relative, [67, -48]);
   writeNegativeRacmoArtifact(data_file);

   dry = icemodel.verification.setup.repairRcmArtifactMetadata( ...
      input_root, eval_root=eval_root, dataset_family="promice", dry_run=true);
   untouched = load(data_file, 'Data');
   testCase.verifyEqual(string(dry.records.status), "would_repair");
   testCase.verifyEqual(untouched.Data.ppt, [-1e-9; NaN; 2e-6]);

   written = icemodel.verification.setup.repairRcmArtifactMetadata( ...
      input_root, eval_root=eval_root, dataset_family="promice", dry_run=false);
   repaired = load(data_file, 'Data', 'artifact_metadata');
   testCase.verifyEqual(string(written.records.status), "repaired");
   testCase.verifyEqual(repaired.Data.ppt, [0; NaN; 2e-6]);
   testCase.verifyEqual(string( ...
      repaired.artifact_metadata.racmo_ppt_qc_status), "applied");
   testCase.verifyEqual( ...
      repaired.artifact_metadata.racmo_ppt_qc_replaced_count, 1);
   testCase.verifyEqual( ...
      repaired.artifact_metadata.racmo_ppt_qc_input_minimum, -1e-9);
   testCase.verifyEqual( ...
      repaired.artifact_metadata.racmo_ppt_qc_output_minimum, 0);

   second = icemodel.verification.setup.repairRcmArtifactMetadata( ...
      input_root, eval_root=eval_root, dataset_family="promice", dry_run=true);
   testCase.verifyEqual(string(second.records.status), "unchanged");
   testCase.verifyEmpty(fieldnames(second.actions));
   reread = load(data_file, 'artifact_metadata');
   testCase.verifyEqual(reread.artifact_metadata, repaired.artifact_metadata);
end
...
function writeRepairManifest(eval_root, data_file, location)
   %WRITEREPAIRMANIFEST Reference one exact RACMO artifact and requested point.
   family_root = fullfile(eval_root, 'promice');
   mkdir(family_root)
   leg = struct('kind', 'point_met', 'staged', true, ...
      'source', 'racmo', 'source_id', 'racmo2.3p3', ...
      'data_files', {{char(data_file)}}, 'sample_method', 'nearest');
   manifest = struct( ...
      'dataset_family', 'promice', ...
      'source_doi', '', 'source_url', '', 'source_version', '', ...
      'retrieval_date', '', ...
      'cases', struct('case_id', 'site', ...
      'site_location', struct('lat_wgs84', location(1), ...
      'lon_wgs84', location(2)), 'colocation', struct('racmo', leg)), ...
      'skipped', []);
...
function writeSentinelArtifact(filename)
   %WRITESENTINELARTIFACT Save a metadata-current artifact with finite 999 data.
   parent = fileparts(filename);
   if ~isfolder(parent)
      mkdir(parent)
   end
   Time = (datetime(2012, 1, 1, TimeZone="UTC") + days(0:2))';
   Data = timetable(Time, 260 + (0:2)', 999 * ones(3, 1), ...
   ...
   Data.Properties.UserData = struct( ...
      ...
      'modis_coverage_years', 2012);
   artifact_metadata = Data.Properties.UserData;
   save(filename, 'Data', 'artifact_metadata')
end

function writeNegativeRacmoArtifact(filename)
   %WRITENEGATIVERACMOARTIFACT Save one tiny invalid canonical ppt channel.
   parent = fileparts(filename);
   if ~isfolder(parent)
      mkdir(parent)
   end
   Time = (datetime(2012, 1, 1, TimeZone="UTC") + days(0:2))';
   Data = timetable(Time, [-1e-9; NaN; 2e-6], 260 + (0:2)', ...
      VariableNames={'ppt', 'tair'});
   save(filename, 'Data')
end
