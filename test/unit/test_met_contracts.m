...
function test_loadmet_gates_promice_filled_by_window_coverage(testCase)
   ...
   % Restore the intact second file. Verify the A5 snow-model extension through
   % the public verifier. A snowfall-consuming smbmodel requires finite total
   % ppt or snowf at every requested timestep. The zero-rain models require
   % only the seven-channel icemodel set.
   met = met_intact2;
   save(hole_file, 'met');
   writePromiceRuntimeManifest(opts);
   snow_opts = opts;
...
function test_loadresults_preserves_diagnostic_output_profile_fields(testCase)
   ...
   testCase.verifyTrue(all(ismember( ...
      {'n_failed_substeps', 'n_forced_advances', 'ea_atm', ...
      'br_coefs_gamma', 'br_coefs_b1_num', ...
   ...
   % Only the solid B/O decomposition still has standing channels; the
   % liquid clone/export split and every enthalpy channel are retired.
   ...
   % The interior transport never enters the surface-exchange values, so
   % the surface closure identity carries no transport term; the
   % transport channels are the transport's own accounting. The exchange
   % draws unsatisfied removal from the cells below the surface, so the
   % demand is realized in full and no remainder term is needed.
