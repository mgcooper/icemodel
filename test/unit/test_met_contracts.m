...
function test_loadresults_preserves_diagnostic_output_profile_fields(testCase)
   ...
   % The interior transport runs after the vapor budget takes its storage
   % baseline, so the surface closure identity carries no redistribution
   % term; those channels are the transport's own accounting.
