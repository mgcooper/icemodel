...
function test_loadresults_preserves_diagnostic_output_profile_fields(testCase)
   ...
   testCase.verifyTrue(all(isfinite(ice1.thf_u_star)));
    ...
    % Diagnostic saves must retain the complete step-ledger contract and its
    % signed storage, remesh, geometry, and phase-aware vapor identities.
    budget_fields = icemodel.column.budget_output_fields();
    testCase.verifyTrue(all(ismember( ...
       budget_fields, ice1.Properties.VariableNames)));
    testCase.verifyTrue(all(isfinite(ice1{:, budget_fields}), 'all'));

    % Every absolute-throughput channel is nonnegative and bounds its signed
    % forcing-step net after accepted substeps and remesh events are accumulated.
    throughput_fields = budget_fields(contains(budget_fields, '_throughput_'));
    testCase.verifyGreaterThanOrEqual(ice1{:, throughput_fields}, 0);
    ...
    testCase.verifyGreaterThanOrEqual( ...
       ice1.mass_budget_solid_storage_throughput_mwe + 1e-12, ...
       abs(solid_delta));
    testCase.verifyGreaterThanOrEqual( ...
       ice1.mass_budget_liquid_storage_throughput_mwe + 1e-12, ...
       abs(liquid_delta));

    signed_throughput_pairs = { ...
...
function test_retimeHourlyFixedStep_uses_cumulative_endpoint_state(testCase)
   ...
   input = timetable(cumulative, cumulative + 100, cumulative + 200, ...
      ...
      'VariableNames', [icemodel.column.cumulative_output_fields(), ...
      {'ordinary'}]);
   ...
   testCase.verifyEqual(icemodel.column.cumulative_output_fields(), ...
