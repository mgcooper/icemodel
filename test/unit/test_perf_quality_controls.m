...
function test_managed_perf_snapshot_refuses_a_noncompliant_source(testCase)
   ...
   % The frozen v1.3 file is tracked, so the refusal must fire before any
   % write and leave that file byte-identical.
   ...
   frozen_digest = icemodel.verification.setup.fileSha256(managed_file);
   ...
   testCase.verifyEqual( ...
      icemodel.verification.setup.fileSha256(managed_file), frozen_digest);
