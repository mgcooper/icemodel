...
function verifyRestartRun(testCase, smbmodel, solver, kwargs)
   ...
   testCase.verifyTrue(all(isfield(restart.restart, ...
      {'T_ice', 'f_ice', 'f_liq', 'T_sfc'})), ...
      'expected restart fields named after the state variables');
   testCase.verifyFalse(any(isfield(restart.restart, {'T', 'Ts'})), ...
      'restart files must not store the state under T or Ts');
