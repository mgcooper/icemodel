...
function test_sfcflux_derivative_matches_finite_difference(testCase)
   % surface_fluxes should return a derivative consistent with a centered finite
   ...
   % Compute the seb residual and derivative for the bulk_richardson scheme
   [Fsfc, Fdot] = ...
      icemodel.surface.turbulence.bulk_richardson.surface_fluxes( ...
      s.tair, s.swd, s.lwd, s.albedo, s.wspd, s.ppt, s.tppt, s.psfc, ...
      s.De, Ts, Qc, s.ea_atm, s.roL, s.br_coefs, s.chi, s.liqflag);

   % Recompute with a small positive Ts perturbation
   h = 1e-5;
   Fplus = icemodel.surface.turbulence.bulk_richardson.surface_fluxes( ...
      s.tair, s.swd, s.lwd, s.albedo, s.wspd, s.ppt, s.tppt, s.psfc, ...
      s.De, Ts + h, Qc, s.ea_atm, s.roL, s.br_coefs, s.chi, s.liqflag);

   % Recompute with a small negative Ts perturbation
   Fminus = icemodel.surface.turbulence.bulk_richardson.surface_fluxes( ...
      s.tair, s.swd, s.lwd, s.albedo, s.wspd, s.ppt, s.tppt, s.psfc, ...
      s.De, Ts - h, Qc, s.ea_atm, s.roL, s.br_coefs, s.chi, s.liqflag);

   % Recompute the derivative using a centered finite difference scheme
...
function test_sfctemp_finds_small_surface_residual(testCase)
   % solve_surface_temperature should find a temperature that leaves only a
   % small residual in the explicit surface-flux balance.

   s = testCase.TestData.skin;

   Qc = CONDUCT(s.k_eff, s.T, s.dz, s.Ts);

   % Solve for T_sfc using the bulk_richardson solver (formerly SFCTEMP)
   [Ts, ok] = ...
      icemodel.surface.turbulence.bulk_richardson.solve_surface_temperature( ...
      s.tair, s.swd, s.lwd, s.albedo, s.wspd, s.ppt, s.tppt, s.psfc, s.De, ...
      s.ea_atm, s.chi, s.roL, s.br_coefs, s.liqflag, Qc);

   % Compute the residual using the bulk_richardson scheme function
   residual = icemodel.surface.turbulence.bulk_richardson.surface_fluxes( ...
      s.tair, s.swd, s.lwd, s.albedo, s.wspd, s.ppt, s.tppt, s.psfc, ...
      s.De, Ts, Qc, s.ea_atm, s.roL, s.br_coefs, s.chi, s.liqflag);

   % Verify the T_sfc solution returned a valid solution
...
function test_sfcflux_includes_precipitation_advection(testCase)
   ...
   Fsfc_dry = icemodel.surface.turbulence.bulk_richardson.surface_fluxes( ...
   ...
   Fsfc_wet = icemodel.surface.turbulence.bulk_richardson.surface_fluxes( ...
...
function test_sebsolve_converges_across_root_finders(testCase)
   ...
   for seb_solver = 0:2
      [Ts, ok] = icemodel.surface.solve_surface_energy_balance( ...
         ...
         s.ppt, s.tppt, s.psfc, s.De, s.ea_atm, s.chi, s.roL, s.br_coefs, ...
      ...
      residual = icemodel.surface.surface_energy_balance_residual(Ts, ...
         s.tair, s.swd, s.lwd, s.albedo, s.wspd, s.ppt, s.tppt, s.psfc, ...
         s.De, s.ea_atm, s.chi, s.roL, s.br_coefs, ...
...
function test_sebsolve_monin_obukhov_converges_with_numeric_derivative(testCase)
   ...
   [Ts, ok] = icemodel.surface.solve_surface_energy_balance( ...
      ...
      s.De, s.ea_atm, s.chi, s.roL, s.br_coefs, s.liqflag, s.T, s.k_eff, ...
      s.dz, s.ro_sfc, s.snow_depth, s.seb_solver, false, s.opts);

   % Compute the residual
   residual = icemodel.surface.surface_energy_balance_residual( ...
      Ts, s.tair, s.swd, s.lwd, s.albedo, s.wspd, s.ppt, s.tppt, s.psfc, ...
      s.De, s.ea_atm, s.chi, s.roL, s.br_coefs, ...
      CONDUCT(s.k_eff, s.T, s.dz, Ts), s.liqflag, s.ro_sfc, s.snow_depth, s.opts);
...
function test_iceebsolvedirichlet_converges_on_synthetic_column(testCase)
   ...
   [Ts, T, f_ice, f_liq, k_eff, ok_seb, ok_ieb, ok, n_iters] = ...
      ...
      s.tair, s.swd, s.lwd, s.albedo, s.wspd, s.ppt, s.tppt, s.psfc, s.De, ...
      s.ea_atm, s.chi, s.roL, s.br_coefs, s.liqflag, s.seb_solver, s.tol, ...
      s.maxiter, s.use_aitken, s.jumpmax, s.cpl_maxiter, s.cpl_Ts_tol, ...
      s.cpl_seb_tol, s.cpl_alpha, s.cpl_aitken, s.cpl_jumpmax, false, ...
   ...
   residual_coupled = icemodel.surface.surface_energy_balance_residual(Ts, ...
      s.tair, s.swd, s.lwd, s.albedo, s.wspd, s.ppt, s.tppt, s.psfc, s.De, ...
      s.ea_atm, s.chi, s.roL, s.br_coefs, CONDUCT(k_eff, T, s.dz, Ts), ...
   ...
   residual_old = icemodel.surface.surface_energy_balance_residual(Ts_old, ...
      s.tair, s.swd, s.lwd, s.albedo, s.wspd, s.ppt, s.tppt, s.psfc, s.De, ...
      s.ea_atm, s.chi, s.roL, s.br_coefs, CONDUCT(k_eff_old, T_old, s.dz, Ts_old), ...
      s.liqflag, s.ro_sfc, s.snow_depth, s.opts);
...
function test_surface_flux_diagnostics_match_energy_balance_residual(testCase)
   ...
   [Ts_raw, ok] = icemodel.surface.solve_surface_energy_balance(s.Ts, s.tair, ...
      ...
      s.chi, s.roL, s.br_coefs, s.liqflag, s.T, s.k_eff, s.dz, s.ro_sfc, ...
      s.snow_depth, s.seb_solver, false, s.opts);
   ...
   [~, ~, ~, ~, Qa, Qle] = icemodel.surface.surface_energy_balance_terms( ...
      Ts, s.tair, s.swd, s.lwd, s.albedo, s.wspd, s.ppt, s.tppt, s.psfc, ...
      s.De, ea_atm, s.T, s.k_eff, s.dz, s.roL, s.chi, s.br_coefs, ...
      s.liqflag, s.ro_sfc, s.snow_depth, s.opts);
   ...
   testCase.verifyEqual(balance, ENBAL(s.chi, s.albedo, s.swd, ...
