...
function test_one_sweep_dirichlet_uses_vapor_free_predictor(testCase)
   ...
   [Ts, T, f_ice, f_liq, U_vap, ~, k_eff, ok_seb, ok_ieb, ok_cpl, cpl_diag] = ...
      ...
      s.cpl_aitken, s.cpl_jumpmax, s.cpl_alpha_min, s.ro_sfc, s.snow_depth, ...
   ...
   [T_x, f_ice_x, f_liq_x, k_x, ~, ~, ~, err_x, U_vap_x] = ...
...
function test_iceebsolvedirichlet_converges_on_synthetic_column(testCase)
   ...
   [Ts, T, f_ice, f_liq, ~, ~, k_eff, ok_seb, ok_ieb, ok_cpl, cpl_diag] = ...
      icemodel.couplers.solve_surface_column_dirichlet( ...
      s.Ts, s.T, s.f_ice, s.f_liq, s.Sc, s.Sp, s.dz, s.delz, s.fn, ...
      s.opts.dt, s.tair, s.swd, s.lwd, s.albedo, s.wspd, s.ppt, s.tppt, ...
      s.psfc, s.ea_atm, s.ro_atm, s.cv_atm, s.nu_air, s.H_h, s.H_e, ...
...
function test_iceenbal_and_iceebsolve_converge_on_synthetic_column(testCase)
   ...
   solver = 3;
   [Ts, T_rob, f_ice_rob, f_liq_rob, ~, ~, k_eff_rob, ~, ok_rob, ~, ...
      diag_rob] = ...
...
function test_robin_adapter_returns_inner_accepted_vapor_flux(testCase)
   ...
   % flux through its fifth output when solver is greater than one.
   ...
   [Fc, Fp] = icemodel.surface.surface_flux_linearization( ...
      ...
      s.snow_depth, s.opts);
   [T_inner, f_ice_inner, f_liq_inner, k_eff_inner, ok_inner, ~, ~, ~, ...
      U_vap_inner] = icemodel.column.solve_column_enthalpy( ...
   ...
   [~, T_adapter, f_ice_adapter, f_liq_adapter, U_vap_adapter, ~, ...
      k_eff_adapter, ok_seb, ok_ieb, ok_cpl] = ...
      icemodel.couplers.solve_surface_column_robin( ...
      s.Ts, s.T, s.f_ice, s.f_liq, s.Sc, s.Sp, s.dz, s.delz, s.fn, ...
      s.opts.dt, s.tair, s.swd, s.lwd, s.albedo, s.wspd, s.ppt, ...
      s.tppt, s.psfc, s.ea_atm, s.ro_atm, s.cv_atm, s.nu_air, s.H_h, ...
      s.H_e, s.hv_atm, s.br_coefs, s.liqflag, s.chi, solver, s.tol, ...
      s.maxiter, s.alpha, s.use_aitken, s.jumpmax, s.cpl_Ts_tol, ...
      s.cpl_seb_tol, cpl_maxiter, s.cpl_alpha, s.cpl_aitken, ...
      s.cpl_jumpmax, s.cpl_alpha_min, s.ro_sfc, s.snow_depth, s.opts.f_res_pore_ice, s.opts);

   testCase.verifyTrue(ok_inner && ok_seb && ok_ieb && ok_cpl);
...
function test_one_cell_column_preserves_solver_and_coupler_outputs(testCase)
   % The eighth solver output must stay a finite residual and the ninth must
   ...
   [T, f_ice, f_liq, k_eff, ok, ~, a1, err, U_vap] = ...
      icemodel.column.solve_column_enthalpy( ...
   ...
   % Both production adapters request output nine and must therefore retain
   % the same one-cell behavior through the complete surface-column call.
   [Ts_dir, T_dir, ~, ~, U_dir, ~, ~, ok_seb_dir, ok_ieb_dir, ok_cpl_dir, ...
      diag_dir] = ...
      icemodel.couplers.solve_surface_column_dirichlet( ...
      s.Ts, s.T, s.f_ice, s.f_liq, s.Sc, s.Sp, s.dz, s.delz, s.fn, ...
      s.opts.dt, s.tair, s.swd, s.lwd, s.albedo, s.wspd, s.ppt, ...
      s.tppt, s.psfc, s.ea_atm, s.ro_atm, s.cv_atm, s.nu_air, s.H_h, ...
      s.H_e, s.hv_atm, s.br_coefs, s.liqflag, s.chi, 1, s.tol, ...
      s.maxiter, s.alpha, s.use_aitken, s.jumpmax, s.cpl_Ts_tol, ...
      s.cpl_seb_tol, 1, s.cpl_alpha, s.cpl_aitken, s.cpl_jumpmax, ...
      s.cpl_alpha_min, s.ro_sfc, s.snow_depth, s.opts.f_res_pore_ice, s.opts);
   testCase.verifyTrue(ok_seb_dir && ok_ieb_dir && ok_cpl_dir);
   testCase.verifyTrue(all(isfinite([Ts_dir; T_dir])));
   testCase.verifyEqual(U_dir, zeros(2, 1), 'AbsTol', 0);

   [Ts_rob, T_rob, ~, ~, U_rob, ~, ~, ok_seb_rob, ok_ieb_rob, ok_cpl_rob, ...
      diag_rob] = ...
      icemodel.couplers.solve_surface_column_robin( ...
      s.Ts, s.T, s.f_ice, s.f_liq, s.Sc, s.Sp, s.dz, s.delz, s.fn, ...
      s.opts.dt, s.tair, s.swd, s.lwd, s.albedo, s.wspd, s.ppt, ...
      ...
      s.cpl_seb_tol, 1, s.cpl_alpha, s.cpl_aitken, s.cpl_jumpmax, ...
      s.cpl_alpha_min, s.ro_sfc, s.snow_depth, s.opts.f_res_pore_ice, s.opts);
   testCase.verifyTrue(ok_seb_rob && ok_ieb_rob && ok_cpl_rob);
   testCase.verifyTrue(all(isfinite([Ts_rob; T_rob])));
   testCase.verifyEqual(U_rob, zeros(2, 1), 'AbsTol', 0);

   % Both couplers return one fixed diag schema, byte-compatible with the
   % shared initializer, so the driver and outputs read them identically.
   expected = fieldnames(icemodel.couplers.initialize_solver_diag());
   testCase.verifyEqual(fieldnames(diag_dir), expected);
   testCase.verifyEqual(fieldnames(diag_rob), expected);
   testCase.verifyEqual(diag_dir.ok_cpl, ok_cpl_dir);
   testCase.verifyEqual(diag_rob.ok_cpl, ok_cpl_rob);
...
function test_robin_debug_dump_handles_inner_failure(testCase)
   ...
   [~, ~, ~, ~, ~, ~, ~, ok_seb, ok_ieb, ok_cpl] = ...
      ...
      s.alpha, s.use_aitken, s.jumpmax, s.cpl_Ts_tol, s.cpl_seb_tol, ...
      s.cpl_maxiter, s.cpl_alpha, s.cpl_aitken, s.cpl_jumpmax, ...
      s.cpl_alpha_min, s.ro_sfc, s.snow_depth, s.opts.f_res_pore_ice, opts);

   % The public flags and persisted diagnostic must identify the same
