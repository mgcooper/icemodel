...
function test_bulk_richardson_dispatch_matches_legacy_flux_kernels(testCase)
   ...
   T_sfc = icemodel.surface.physical_surface_temperature(s.T_sfc);
...
function test_monin_obukhov_cold_state_remains_continuous(testCase)
   ...
   % Its fluxes must vary smoothly across small T_sfc perturbations.
...
function test_monin_obukhov_linearization_matches_finite_difference(testCase)
   ...
   h = 1e-5;
   q_plus = surface_flux_bulk_mo(s.T_sfc + h, s);
   q_minus = surface_flux_bulk_mo(s.T_sfc - h, s);
   dq_fd = (q_plus - q_minus) / (2 * h);

   testCase.verifyTrue(all(isfinite([Fc, Fp, diag.q_surface, ...
      diag.dq_surface_dT_sfc])));
   testCase.verifyEqual(Fc + Fp * s.T_sfc, diag.q_surface, 'RelTol', 1e-10);
...
function test_vapor_demand_uses_physical_surface_temperature(testCase)
   % Potential vapor demand should evaluate Qe using the physical
   % diagnosed surface temperature even if the solver-internal T_sfc exceeds Tf.

   s = icemodel.test.fixtures.makeSyntheticColumnState( ...
      testCase.TestData.workspace, 'icemodel', solver=1, seb_solver=2, ...
      turbulent_flux_scheme='monin_obukhov', z0_ice=0.02, ...
      testname='potential_vapor_demand');
   Lv = icemodel.physicalConstant('Lv');

   T_sfc_solver = s.Tf + 3.0;

   [d_pevp, pevp, Qe, T_sfc_phys] = ...
      icemodel.surface.potential_surface_vapor_demand(T_sfc_solver, s.tair, ...
   ...
   [Qe_phys, ~] = icemodel.surface.diagnose_turbulent_heat_fluxes( ...
      icemodel.surface.physical_surface_temperature(T_sfc_solver), ...
      s.tair, s.wspd, s.psfc, s.ea_atm, s.ro_atm, s.cv_atm, s.nu_air, ...
      s.H_h, s.H_e, s.hv_atm, s.br_coefs, s.liqflag, s.ro_sfc, ...
      s.snow_depth, s.opts);

   [Qe_uncapped, ~] = icemodel.surface.diagnose_turbulent_heat_fluxes( ...
      T_sfc_solver, s.tair, s.wspd, s.psfc, s.ea_atm, s.ro_atm, s.cv_atm, ...
      s.nu_air, s.H_h, s.H_e, s.hv_atm, s.br_coefs, s.liqflag, s.ro_sfc, ...
      s.snow_depth, s.opts);

   testCase.verifyEqual(T_sfc_phys, s.Tf, 'AbsTol', 1e-12);
...
function test_thf_debug_dump_reuses_diag_contract(testCase)
   ...
   icemodel.surface.dump_turbulent_heat_flux_debug_state( ...
      'unit_test', s.T_sfc, s.T_sfc - 0.5, s.tair, s.swd, s.lwd, s.albedo, ...
      s.wspd, s.ppt, s.tppt, s.psfc, s.ea_atm, s.ro_atm, s.cv_atm, ...
      s.nu_air, s.H_h, s.H_e, s.hv_atm, s.br_coefs, s.liqflag, s.chi, ...
      s.T_ice, s.k_eff, s.dz, s.ro_sfc, s.snow_depth, s.opts);

   loaded = load(debug_file, 'debug_state');
   debug_state = loaded.debug_state;

   testCase.verifyEqual(debug_state.reason, 'unit_test');
   testCase.verifyEqual(debug_state.turbulent_flux_scheme, 'monin_obukhov');
   testCase.verifyEqual(debug_state.thf_at_T_sfc.diag.scheme, 'monin_obukhov');
