...
function test_fetchEsmSnowmip_strict_errors_on_empty_cache(testCase)
   ...
   % files are present. Run the check under evalc so the retrieval
   % banner text stays out of the suite log, and confirm the banner
   % still prints for interactive users by asserting on the capture.
   captured = evalc(['testCase.verifyError(@() ' ...
   ...
   testCase.verifySubstring(string(captured), "source cache incomplete");
end
...
function test_fetchLaughTests_strict_errors_on_empty_cache(testCase)
   % Run the check under evalc so the retrieval banner text stays out of
   % the suite log, and confirm the banner still prints for interactive
   % users by asserting on the capture.
   captured = evalc(['testCase.verifyError(@() ' ...
      'icemodel.verification.setup.fetchLaughTests(' ...
      'cache_dir=string(testCase.TestData.tmp), ' ...
      'strict=true, silent=true), ' ...
      '''icemodel:verification:fetchLaughTests:missingSources'');']);
   testCase.verifySubstring(string(captured), "source checkout incomplete");
end
